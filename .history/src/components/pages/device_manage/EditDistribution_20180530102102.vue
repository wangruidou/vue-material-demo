<template>
    <div class="flex-content">
        <div class="flex-item device-type-tree">
            ...
            <TypeTree :rootName="rootName" :selectedPath="selectedPath" :children="typeData" v-on:onNodeSelectedChange="onNodeSelectedChange"></TypeTree>
        </div>
...
export default {
    ...
    methods: {
        ...
        changeDevice () {
            ...
            console.log(this.selectedOnTrain)
            if (this.selectOnTrain) {
                const index = this.allDevicesOnTrain.indexOf(this.selectOnTrain)
                if (index >= 0) {
                    this.allDevicesOnTrain.splice(index, 1)
                }
            }
            console.log(this.selectedUnderTrain)
            if(this.selectedUnderTrain) {
                const index = this.localDevices[this.selectedNodeid].indexOf(this.selectedUnderTrain)
                if (index >= 0) {
                    this.localDevices[this.selectedNodeid].splice(index, 1)
                }
            }
            
            this.allDevicesOnTrain.push(this.selectedUnderTrain)
        },
        removeDevice () {
            if(this.$utils.isEmptyObject(this.selectedOnTrain)) {
                this.snackbarActive = true

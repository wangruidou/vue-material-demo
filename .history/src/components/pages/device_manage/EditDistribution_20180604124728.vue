...
const deleteChildren = (topParentDeviceId, currentParentId, currentDeviceid) => {
    let parentItem = this.deviceData[this.deviceDataIndex[this.$utils.trainsIdToKey(currentParentId)]]
    let this_ = this
    while(parentItem.deviceid == topParentDeviceId) {
        ...
        parentItem = this_.deviceData[this_.deviceDataIndex[this_.$utils.trainsIdToKey(parentItem.parentid)]]
    }
    let index = parentItem.children.indexOf(currentDeviceid)
    if(index >= 0) {
        parentItem.children.splice(index, 1)

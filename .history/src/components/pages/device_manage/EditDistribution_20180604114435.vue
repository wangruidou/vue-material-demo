...
const getParentid = (treeTypeData, childid) => {
    ...
    methods: {
        ...
        dialogConfirm () {
            if(this.underDeviceHasParent == true || this.changeOnUnder == true) {
                ...
                if(currentParentid == this.$route.params.item.deviceid) { // 如果删除的为子级
                    let i = this.$route.params.item.children.indexOf(this.selectedOnDevice.deviceid)
                    if (i >= 0){
                        this.$route.params.item.children.splice(i, 1)
                    }
                    this.$route.params.item.children.push(this.selectedUnderDevice.deviceid)
                } else { // 删除的为孙级
                    let parentItem = this.$store.state.deviceData[this.$store.state.deviceDataIndex[this.$utils.trainsIdToKey(currentParentid)]]
                    let i = parentItem.indexOf(this.selectedOnDevice.deviceid)
                    if (i >= 0){
                        parentItem.splice(i, 1)
                    }
                    parentItem.push(this.selectedUnderDevice.deviceid)
                }

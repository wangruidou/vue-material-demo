<template>
    <div :params="params" :points="points" :deviceKinds="deviceKinds">
        <table class="tab-con">
            <tr v-for="(value, key) in tabTitle" :key="key" style="height: 50px">
                <td class="tab-title">
                    {{value}}
                </td>
                <td>
...
export default {
    ...
    methods: {
        ...
        saveDlg () {
            let result = []
            result.push(this.filterKinds)
            result.push(this.filterParams)
            result.push(this.filterPoints)
            this.$emit('saveDlg', result)
        }
    },
    created() {

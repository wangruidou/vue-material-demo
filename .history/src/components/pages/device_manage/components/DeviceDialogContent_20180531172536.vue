    ...
    <div :points="points" :deviceKinds="deviceKinds" :defaultValue="defaultValue">
        ...
        <table class="tab-con">
            <tr v-for="(item, key) in points" :key="key">
                ...
                <td>
                    <md-field>
                        <md-select v-if="item.pointType == 'Select'" v-model="filterparam.point[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" placeholder="请选择..">
                            <md-option v-for="(v, k) in item.enable" :value="v" :key="k">{{v}}</md-option>
                        </md-select>
                        <md-select v-else-if="item.pointType == 'Boolean'" v-model="filterparam.point[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" placeholder="请选择..">
                            <md-option :value="item.falseValue">{{item.falseValue}}</md-option>
                            <md-option :value="item.trueValue">{{item.trueValue}}</md-option>                            
                        </md-select>
                        <md-input v-else-if="item.pointType == 'Number'" type="number" v-model="filterparam.point[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" placeholder="请输入.."></md-input>
                        <md-input v-else v-model="filterparam.point[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" placeholder="请输入.."></md-input>                    
                    </md-field>
                </td>
            </tr>
        </table>

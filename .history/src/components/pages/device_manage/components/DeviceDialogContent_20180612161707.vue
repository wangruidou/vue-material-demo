            ...
            <tr v-for="(value, key) in tabTitle" :key="key">
                ...
                <td>
                    <md-field>
                        ...
                        <md-select v-else-if="key=='devicekindid'" v-model="filterparam[key]" :name="key" :id="key" placeholder="请选择..">
                            <md-option v-for="item in deviceKinds" :value="item.id" :key="item.id">{{item.name}}</md-option>
                        </md-select>
                    </md-field>
                </td>
            </tr>

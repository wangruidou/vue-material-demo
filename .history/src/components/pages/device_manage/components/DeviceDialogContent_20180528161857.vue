    ...
    <div :params="params" :points="points" :deviceKinds="deviceKinds">
        ...
        <table class="tab-con">
            <tr v-for="(item, key) in params" :key="key">
                ...
                <td>
                    <md-field>
                        <md-select v-model="filterParams[item.id]" :name="item.id" :id="item.id"
        ...
        <table class="tab-con">
            <tr v-for="(item, key) in points" :key="key">
                ...
                <td>
                    <md-field>
                        <md-select v-if="item.pointType == 'Select'" v-model="filterParams[item.id]" :name="item.id" :id="item.id"
                            @md-selected="handleSelectedParams">
                            <md-option v-for="(v, k) in item.enable" :value="v" :key="k">{{v}}</md-option>
                        </md-select>
                    </md-field>

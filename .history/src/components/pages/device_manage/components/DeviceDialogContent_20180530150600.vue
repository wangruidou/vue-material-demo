    ...
    <div :points="points" :deviceKinds="deviceKinds" :defaultValue="defaultValue">
        ...
        <table class="tab-con">
            <tr v-for="(item, key) in points" :key="key">
                ...
                <td>
                    <md-field>
                        <md-select v-if="item.pointType == 'Select'" v-model="filterparam[$utils.trainsIdToKey(item.id)]" 
                        ...
                        <md-select v-else-if="item.pointType == 'Boolean'" v-model="filterparam[$utils.trainsIdToKey(item.id)]" 
                            :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" placeholder="请选择..">
                            <md-option :value="item.falseValue">{{item.falseValue}}</md-option>
                            <md-option :value="item.trueValue">{{item.trueValue}}</md-option>                            
                        </md-select>

                        <md-input v-else-if="item.pointType == 'Number'" type="number" v-model="filterparam[$utils.trainsIdToKey(item.id)]" 
                        ...
                        <md-input v-else v-model="filterparam[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" 

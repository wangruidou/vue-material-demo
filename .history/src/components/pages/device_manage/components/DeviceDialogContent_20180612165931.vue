            ...
            <tr v-for="(value, key) in tabTitle" :key="key">
                ...
                <td>
                    <md-field v-if="key=='devicecode'">
                        <md-input v-model="filterparam[key]" placeholder="请输入.."></md-input>
                    </md-field>
                    <md-field v-else-if="key=='devicekindname'">
                        <md-select v-model="filterparam[key]" :name="key" :id="key" placeholder="请选择..">
                            <md-option v-for="item in deviceKinds" :value="item.name" :key="item.id">{{item.name}}</md-option>
                        </md-select>
                    </md-field>
                    <md-field v-else-if="key=='productiondate'">    
                        <md-input v-model="filterparam[key]" placeholder="请输入.."></md-input>
                    </md-field>
                    <md-datepicker v-else-if="key=='unitid'" v-model="filterparam[key]" placeholder="请选择.." md-immediately />
                </td>
            </tr>
        </table>
        <div class="tab-btn">

            ...
            <tr v-for="(value, key) in tabTitle" :key="key">
                ...
                <td>
                    <md-field>
                        ...
                        <md-select v-else-if="key=='devicetype'" v-model="filterparam[key]" :name="key" :id="key" placeholder="请选择..">
            ...
            <tr v-for="(item, key) in points" :key="key">
                ...
                <td>
                    <md-field>
                        ...
                        <md-input v-else-if="item.pointType == 'Number'" type="number" v-model="filterparam[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" :autocomplete="$utils.trainsIdToKey(item.id)" placeholder="请输入.."></md-input>
                        <md-input v-else v-model="filterparam[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" :autocomplete="$utils.trainsIdToKey(item.id)" placeholder="请输入.."></md-input>                    
                    </md-field>
                </td>
            </tr>
        </table>
...
export default {
    ...
    methods: {
        ...
        saveDlg () {
            this.$emit('saveDlg', Object.assign(this.filterparam, this.filterparam))
        }
    },
    computed: {
        filterparam () {

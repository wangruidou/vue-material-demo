    ...
    <md-card class="screen-tab" v-if="isShow" :kindsData="kindsData" :paramsData="paramsData" :pointsData="pointsData">
        ...
        <table class="tab-con">
            <tr v-for="item in paramsData" :key="item.id">
                ...
                <td>
                    <md-field>
                        <md-select v-model="filterParams[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" @md-selected="handleSelectedParams">
                            <md-option value="none">全部</md-option>
        ...
        <table class="tab-con">
            <tr v-for="item in pointsData" :key="item.id">
                ...
                <td>
                    <md-field>
                        <md-select v-if="item.pointType == 'Select'" v-model="filterPoints[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" @md-selected="handleSelectedParams">
                            <md-option value="none">全部</md-option>
                            <md-option v-for="(v, k) in item.enable" :value="v" :key="k">{{v}}</md-option>                            
                        </md-select>
                        <md-select v-else-if="item.pointType == 'Boolean'" v-model="filterPoints[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)" @md-selected="handleSelectedParams">
                            <md-option value="none">全部</md-option>

            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }" :class="getClass(item)" md-selectable="single">
                ...
                <md-table-cell md-label="更多信息">
                    <md-button class="md-icon-button">
                        <md-icon @click="moreInfos">navigate_next</md-icon>
                    </md-button>
                </md-table-cell>
            </md-table-row>
        </md-table>
...
export default {
  ...
  data: () => ({
    ...
    deviceDialogTitle: '',
    selectedItem: null,
    selectedItem_: [], // 取消选择状态时selectedItem为null，记录不为null的值
    selectedItemKinds: [],
    selectedItemParams: [],
    selectedItemPoints: [],
    paramsObj: {},
  ...
  methods: {
    ...
    moreInfos () {
        if (!this.$utils.isEmptyObject(this.selectedItem_)) {
            this.$router.push({
                path: '/DeviceDetail',
                name: 'DeviceDetail',
                params: {
    ...
    saveDlg (value) {
        if(this.$utils.isEmptyObject(this.defaultValue)){ // 添加
            value.typeid = this.currendNodeid
            this.snackbarActive = true
            this.snackbarInfo = '添加成功'
            this.searched.push(value)
        } else { // 修改
            let index = this.deviceData.indexOf(this.selectedItem)
      ...
      selectedNode: function (newValue, oldValue) {
          ...
          if (this.deviceData) {
            this.deviceData.forEach(function (item) {
                if (this_.currendNodeid == item.devicetypeid){
                    devices.push(item)
                }
            })
          }

      ...
      <div class="flex-item">
        <md-table v-model="searched" md-card md-fixed-header @md-selected="onSelect">
            <md-table-toolbar>
                ...
                <md-field title="上传文件" style="width: 200px;" @md-change="handleImportFile">
                    <md-file v-model="importInfos" />
                </md-field>
            </md-table-toolbar>
...
export default {
  ...
  methods: {
    ...
    handleSelectedParams (value) {
        console.log(value)
        console.log(this.filterParams['_136a3d6f_6f9d_4adc_830c_9b62498f45ec'])
    }
  },
  watch: {
          ...
          handler: function () {
            if (!isEmptyObject(this.filterParams)) {
                ...
                this.selectedItemParams.forEach(function (item) {
                    let param = this_.$utils.trainsIdToKey(item.id)
                    console.log(param)
                    console.log(this_.filterParams[param])
                })
            }
        },
        deep: true

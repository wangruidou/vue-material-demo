...
const deleteChildren = (topParentDeviceId, currentParentId, currentDeviceid) => {
    let parentItem = this.$store.state.deviceData[this.$store.state.deviceDataIndex[this.$utils.trainsIdToKey(currentParentId)]]
    let this_ = this
    while(parentItem.deviceid == topParentDeviceId) {
        let index = parentItem.children.indexOf(currentDeviceid)
        if(index >= 0) {
            parentItem.children.splice(index, 1)
        }
        parentItem = this_.$store.state.deviceData[this_.$store.state.deviceDataIndex[this_.$utils.trainsIdToKey(parentItem.parentid)]]
    }
    let index = parentItem.children.indexOf(currentDeviceid)
    if(index >= 0) {
        parentItem.children.splice(index, 1)
...
export default {
    ...
    methods: {
        ...
        dialogConfirm () {
            if(this.underDeviceHasParent == true || this.changeOnUnder == true) {
                ...
                deleteChildren(this.$route.params.item.deviceid, currentParentid, this.selectedOnDevice.deviceid)
                console.log(this.$store.state.deviceData[this.$store.state.deviceDataIndex[this.$utils.trainsIdToKey(currentParentid)]].children)
    ...
    created () {
        ...
        this.$route.params.item.children.forEach(function(item) {
            onDeviceIndex.push(this_.$store.state.deviceDataIndex[this_.$utils.trainsIdToKey(item)])
            this_.allOnCurrentDevice.push(this_.$store.state.deviceData[this_.$store.state.deviceDataIndex[this_.$utils.trainsIdToKey(item)]])            
        })        

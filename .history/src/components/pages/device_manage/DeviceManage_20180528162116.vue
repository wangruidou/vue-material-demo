  ...
  <div class="flex-content">
      ...
      <md-dialog :md-active.sync="showDialog" class="device-dialog-con">
          ...
          <md-content class="md-scrollbar device-content">
              <DeviceDialogContent :deviceKinds="selectedItemKinds" :params="selectedItemParams" :points="selectedItemPoints"></DeviceDialogContent>
          </md-content>
          <md-dialog-actions>
            <md-button class="md-raised md-accent" @click="showDialog = false">关闭</md-button>
            <md-button class="md-raised md-primary" @click="showDialog = false">保存</md-button>
          </md-dialog-actions>
      </md-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    editChildren () {        
        if (this.selectedItem == null) {
            ...
            this.$router.push({path: '/EditDistribution/' + id + '/' + typeid})
        }
    },
    handleScrap () {
        alert("设备报废")

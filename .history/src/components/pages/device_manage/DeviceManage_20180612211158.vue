      ...
      <div class="flex-item">
        <md-table v-model="searched" md-card md-fixed-header @md-selected="onSelect">
            <md-table-toolbar>
                ...
                <md-button class="md-icon-button" title="aaa" @click="test">
                    aaa
                </md-button>
            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }" :class="getClass(item)" md-selectable="single">
                ...
                <md-table-cell md-label="已使用时长">{{ item.firstusedate }}</md-table-cell>
...
export default {
  ...
  methods: {
    test: function() {
        let _this = this
        setInterval(function(){_this.$store.commit("test")},1000)
    },
    getClass: ({ typeid }) => ({
        'md-primary': typeid != ''
    }),

...
const getParentid = (treeTypeData, childid) => {
    ...
    treeTypeData.forEach(function(item) {
        if (item.model.typeid == childid) {
            return item.model.parentid
        } else {
            return getParentid(item.children, childid)
        }
    })
}
...
export default {
    ...
    created () {
        ...
        console.log(getParentid (this.typeData, '0111'))
    },    
    watch: {

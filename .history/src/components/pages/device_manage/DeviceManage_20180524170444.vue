        ...
        <md-card class="screen-tab" v-show="isShow">
            ...
            <md-field class="screen-content">
                ...
                <md-select v-model="deviceModel" name="deviceModel" id="deviceModel" @md-selected="handleSelected">
                    ...
                    <md-option v-for="item in selectedItemKinds" :value="item.id" :key="item.id">{{item.name}}</md-option>
                </md-select>
            </md-field>
...
export default {
  ...
  computed: {
    ...
    deviceData () {
        this.paramsObj = this.$store.state.deviceData[0]
      ...
      paramsObj: {
          handler(val, oldVal) {
              ...
              if(!isEmptyObject(keys)) {
                  this.searched.forEach(function(item) {
  ...
  created() {
    if(!isEmptyObject(this.deviceData)){
        let keys = []
        let this_ = this
        let device = this.deviceData[0]
        for(let k in device) {
            this.deviceData.hasOwnProperty(k)
            keys.push(k)
        }
        keys.forEach(function(v) {
            this_.paramsObj[v] = ''

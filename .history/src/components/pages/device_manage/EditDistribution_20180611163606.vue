    ...
    methods: {
        ...
        dialogConfirm () {
            if(this.underDeviceHasParent == true || this.changeOnUnder == true) {
                ...
                let pid = this.selectedUnderDevice.parentid
                let ptypeid = this.$store.state.deviceData[this.deviceDataIndex[this.$utils.trainsIdToKey(pid)]].devicetypeid
                if (this.selectedNodeid == ptypeid) {
                    alert("1")
                } else {
                    alert("2")
                }

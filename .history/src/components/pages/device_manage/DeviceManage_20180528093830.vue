      ...
      <div class="flex-item">
        ...
        <md-card class="screen-tab" v-show="isShow">
            <md-content class="md-scrollbar md-content-screen">
                ...
                <md-field class="screen-content" v-for="item in selectedItemParams" :key="item">
                    ...
                    <md-select v-model="filterParams[$utils.trainsIdToKey(item.id)]" :name="$utils.trainsIdToKey(item.id)" :id="$utils.trainsIdToKey(item.id)"
                        ...
                        <md-option value="none">全部</md-option>
                        <md-option v-for="v in item.enable" :value="v" :key="v">{{v}}</md-option>
                    </md-select>
                </md-field>
                <md-divider style="margin-top: 10px"></md-divider>
                <md-field class="screen-content" v-for="item in selectedItemPoints" :key="item">
                    <label>{{item.name}}</label>
  ...
  computed: {
    ...
    selectedNode () {
        ...
        if(node){
            ...
            for (let i in node.model.points) {
                let k = this.$utils.trainsIdToKey(node.model.points[i].id)
                let type = node.model.points[i].pointType

        ...
        <md-table v-model="searched" md-card md-fixed-header @md-selected="onSelect">
            <md-table-toolbar>
                <div class="md-toolbar-section-start">
                    <h3>设备列表</h3>
                    <md-button class="md-raised" :class="screenBtnClass" @mouseover="isShow = true">{{screenBtn}}</md-button>
                </div>
  ...
  methods: {
    ...
    handleSelectedParams (value) {
        ...
        this.selectedItemParams.forEach(function (item) {
            ...
            if(selectValue != "none"){
                param_key.push(param)
                param_value.push(selectValue)
            }
        })

        let selectData = []
        this.paramSearched = this.searched
        this.devices.forEach(function(v) {
            let flag = true
            for(let k in param_key){
                if(v[param_key[k]] == param_value[k]){
                    flag = true
                } else{
                    flag = false
                }

...
export default {
    ...
    computed: {      
        ...
        rootName () {
            let childs = this.$route.params.item.children
            for(let i=0;i<childs.length;i++) {
                this.childrenItem.push(this.$store.state.deviceData[this.deviceDataIndex[this.$utils.trainsIdToKey(childs[i])]]) // 当前设备的全部子设备
            }

<template>
  <li class="device-type-tree-node" >
    ...
    <ul v-if="nodeExtends && children.length>0 " v-for="(child, index) in children">
      <device-type-tree-node :value="child.value" :children="child.children" :nodeModel="child.model"  v-on:childClick="childClick" v-on:deleteChild="deleteChild" :index="index"/>
    </ul>
  </li>
</template>

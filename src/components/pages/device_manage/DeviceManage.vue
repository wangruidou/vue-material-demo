<template>
  <div class="flex-content">
    <div class="flex-item device-type-tree">
      <TypeTree class="type-tree-content" rootName="全部种类" :selectedPath="selectedPath" :children="typeData"
                v-on:onNodeSelectedChange="onNodeSelectedChange"></TypeTree>
    </div>
    <div class="flex-item">
      <md-table v-model="searched" md-card md-fixed-header @md-selected="onSelect">
        <md-table-toolbar>
          <div class="md-toolbar-section-start">
            <h3>设备列表</h3>
            <md-button class="md-raised md-primary" @click="currentNodeid != '' ? (isShow = true) : (isShow = false)">
              筛选数据
            </md-button>
          </div>
      ...
      <DeviceScreen :isShow="isShow" v-on:closeCard="isShow = false" :kindsData="selectedItemKinds"
                    v-on:handleSelected="handleSelected" :paramsData="selectedItemParams"
                    :pointsData="selectedItemPoints"
    ...
    methods: {
      ...
      editChildren() {
        if (this.selectedItem == null) {
          ...
          let ary = []
          ary.push(this.$utils.getTypeTreeData(this.typeData, typeid))
          if ((!'children' in ary[0]) || ary[0].children.length == 0) {
      ...
      saveDlg(value) {
        if (this.$utils.isEmptyObject(this.defaultValue)) { // 添加
          ...
          value.productiondate = new Date(this.$utils.formatDate(date, 'yyyy-MM-dd hh:mm')).getTime()
          value.createdate = new Date(this.$utils.formatDate(new Date(), 'yyyy-MM-dd hh:mm')).getTime()
          value.firstusedate = 0
          let this_ = this
          this.$ajax.post('data/handle', 'msg=' + JSON.stringify(value))
            .then(function (response) {
              if (response.data.errorcode == 0) {
                value.typeid = this_.currentNodeid
                this_.snackbarActive = true
                this_.snackbarInfo = '添加成功'
                value.createdate = this_.$utils.formatDate(new Date(), 'yyyy-MM-dd hh:mm')

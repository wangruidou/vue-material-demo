        ...
        <div class="detail-header">
            ...
            <md-table v-model="currentItem" md-card>
                <md-table-row slot="md-table-row" slot-scope="{ item }">
                    ...
                    <md-table-cell md-label="建档日期">{{ item.createdate == 0 ? "无" : $utils.trainsDataBetweenNow(item.createdate) }}</md-table-cell>
                    <md-table-cell md-label="已使用时长">{{ item.firstusedate == 0 ? "" : $utils.trainsDataBetweenNow(item.firstusedate) }}</md-table-cell>
...
export default {
    ...
    computed: {
        ...
        currentParam() {
          let param = this.$route.params.model.params
          return param
        },
        currentPoint() {
          let point = this.$route.params.model.points
          return point
        },
        currentModel() {
